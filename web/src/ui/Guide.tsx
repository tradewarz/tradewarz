  ...
  return (
    <div class="guide">
      ...
      <article class="guide-body">
        ...
        <p>One brake sits with the site: during an incident the owner can pause buying for everyone. A banner says so, bots stop entering and Buy is switched off — while exits, sells and withdrawals carry on as normal. It is a pause, not a freeze: nothing you hold is ever locked.</p>
        ...
        <dl class="facts">
          ...
          <dt>TPM</dt><dd>Live trade velocity: trades per minute over the last minute, with the 5-minute average beside it. Counted from real prints on the hub stream (Robinhood curve trades, Solana trades when that feed is on), not from a % spike. A fat 5-minute window with no 1-minute tape yet shows as an approximate <i>~N</i>. <b>LIT</b> means the tape just crossed from quiet into hot this minute (calibrated so 2–3 thin ticks never light it; IPO-style tape is tens of tx/min). <b>HOT</b> means it is still printing hard. Click the header to sort. The <b>Hot TPM</b> filter keeps only those rows.</dd>
        ...
        <p>The filter buttons: <b>All</b>, <b>Passing</b> (PASS and wait), <b>Traded</b> (held or sold), <b>★ Watching</b> (your starred coins, kept even after the hub stops streaming them), <b>Bundled</b>, <b>Hot TPM</b> (tokens whose tape is hot or just lit). "Buy by address" opens the buy dialog for any token you paste. The "alerts" button turns on a tone and desktop notifications for passes while a bot is off, buys and sells, bundles on coins you hold or watch, a tape that just lit, and errors.</p>
        <h3 id="g-drawer">The drawer</h3>
        <p>Clicking a row opens the drawer: the verdict with every failed rule spelled out, and every fact the hub has — age, source, liquidity, market cap, price, tx velocity, volume, buys and sells, safety, the launch facts, the listing score. <b>Buy…</b> opens the buy dialog; nothing is sent until you confirm there. <b>☆ Watch</b> stars it.</p>
        ...
        <p><b>Signing in on a phone:</b> phones have no wallet extensions, so nothing on a phone can hand the page Phantom or MetaMask. Two ways in. Either open TradeWarz <i>inside</i> the wallet app's own browser — the sign-in buttons on a phone become "Open in the Phantom app" / "Open in the MetaMask app", and inside those browsers everything works as on a computer. Or, on a computer that is signed in, go to Account → "Use it on another device", make a one-time code, and type it (or open its link) on the phone: the phone is then signed into the same account with no wallet at all. Codes last five minutes and work once.</p>
